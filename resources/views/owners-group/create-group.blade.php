      ...
      <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
        <div class="">
          ...
          <form action="{{route('create-group.store')}}" class="w-full mx-auto shadow-xl p-6 rounded-lg" method="POST">
            @csrf
            <div class="w-full">
              <x-label for="name" class="mb-2 text-left">Community Name:</x-label>
              <x-input type="text" name="name" id="name" class="w-full" placeholder="Community Name"/>
              <x-input-error for="name" class="mt-2"></x-input-error>
            </div>
            
            <x-button class="mt-5 bg-green-600 hover:bg-green-700">Create Community</x-button>
          </form>
        </div>
      </div>
    </div>
