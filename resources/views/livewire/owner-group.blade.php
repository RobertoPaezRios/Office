                        ...
                        <div class="w-64 h-72 rounded-lg shadow-xl border hover:opacity-75 transition duration-300 hover:cursor-pointer">
                            <div class="rounded-t-lg h-20 p-3 text-white truncate" style="background-color: {{$colors[$group->id]}}">
                                <div class="flex justify-between">
                                    ...
                                    <div class="">
                                        @if (count($group->teams) == 0 && count($partners[$group->id]) == 0)
                                            <button wire:click="setDeleteUuid('{{$group->uuid}}')" data-bs-toggle="modal" data-bs-target="#confirmateModal" class="rounded-full p-1 hover:bg-red-700 transition duration-150">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                ...
                                <div class="text-md overflow-hidden text-ellipsis flex justify-between">
                                    <span class="my-auto truncate">
                                        {{$owners[$group->id]->name}}
                                    </span>
                                    <div>
                                        <div class="flex px-1 -space-x-1">
                                            @foreach ($partners[$group->id] as $key => $partner)
                                                <img class="w-6 h-6 border border-black rounded-full" src="{{\App\Models\User::find($partner->user_id)->profile_photo_url}}" alt="">
                                            
                                                @if ($key == 4)
                                                    <div class="flex items-center justify-center w-6 h-6 text-xs font-medium text-white bg-gray-700 border border-black rounded-full hover:bg-gray-600">
                                                        {{count($partners[$group->id]) - 4}}
                                                    </div> 
                                                @endif
                                            @endforeach
                                        </div>
